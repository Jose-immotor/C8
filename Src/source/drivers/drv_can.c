...
//	can_struct_para_init(CAN_INIT_STRUCT, &can_init_parameter);
	...
	/*
		...
		²¨ÌØÂÊ: CANÊ±ÖÓ/((1+BS1+BS2) * Prescaler) 
		²ÉÑùµã: (1+BS1)/(1+BS1+BS2)

		²¨ÌØÂÊ: 24M /( ( 1 + 3 + 2) * 18 ) = 250K
		²ÉÑùµã£º( 1 + 3)/( 1 + 3 + 2 ) = 66%
	*/
...
//	can_struct_para_init(CAN_INIT_STRUCT, &can_init_parameter);
    ...
    can_filter_init(&can_filter_parameter);
// can1_filter_start_bank(14);
    /* initialize transmit message *///æ‹“å±•æ•°æ®å¸?  tx_efidå‰?ä¸ªå­—èŠ‚ä¸ºæŽ¥å—æ–¹åœ°å€ï¼ŒåŽ8ä¸ªå­—èŠ‚ä¸ºè‡ªå·±åœ°å€
 //   transmit_message.tx_sfid = 0x0010;
    transmit_message.tx_efid = CAN_TX_ID;
    transmit_message.tx_ft = CAN_FT_DATA;
    transmit_message.tx_ff = CAN_FF_EXTENDED;
