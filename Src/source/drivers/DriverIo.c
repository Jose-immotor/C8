...
//	,{CTRL_AC5		, "CTRL_12V5_ON"		, GPIOB, GPIO_PIN_4  ,GPIO_MODE_OUT_PP, 0, 0, 0, 0, 0, 0, 0}
	...
	//,{CTRL_485_COOLING_ENABLE, "CTRL_485_COOLING"	, GPIOA, GPIO_PIN_8  ,GPIO_MODE_OUT_PP}	//和TIME0_CH0的PA8复用，用于触发ADC检测
...
void IO_Init()
{
	...
	for (int i = 0; i < GET_ELEMENT_COUNT(g_InIOs); i++)
	{
		...
		if(g_InIOs[i].ioID == DET_WATER_IN) g_InIOs[i].ticksDelay = 2000;	//特殊防抖
	}
