...
typedef struct _Utp
{
	...
	uint16_t m_txBufLen;
	uint8_t m_txBuf[UTP_REQ_BUF_SIZE];	//存放发送出去数据，转码前
	uint8_t m_rxBuf[UTP_RSP_BUF_SIZE];	//存放接收到的RAW数据，转码前
	Queue m_rxBufQueue;					//管理m_rxBuf的。

	int   m_frameLen;
	uint8_t m_Frame[UTP_RSP_BUF_SIZE];//存放接收到的数据，m_rxBuf转码后的帧数据
