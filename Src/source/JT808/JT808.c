...
Bool gJT808UpdateFirmware = False ;	// Çå³ýÖ®

Bool gJT808Disconnect = False ;		// JT808ÊÇ·ñ¶Ï¿ª,Á¬Ðø1·ÖÖÓÎ´ÊÕµ½CANÊý¾Ý,ÔòÈÏÎªÆä¶Ï¿ª
...
static Bool _CheckFirmwareUpdate(void)
{
	...
	return pAdc->newValue > 3900 ? true : false ;	// > 3.9v Ôò¿ÉÒÔÉý¼¶
	
	// Ö»ÒªµçÁ¿ > 3.9v Ôò¿ÉÒÔ¿ªÊ¼Éý¼¶
	...
	else if(ev == UTP_REQ_SUCCESS )
	{
		...
		if( pJt->filecontentreq.fileOffset == pJt->filecontentrsq.fileOffset )
		{
			...
			if( pJt->filecontentreq.fileOffset >= pJt->updatefileinfo.Updatefilelength )
			{
				...
				{
					gJT808UpdateFirmware = true ;	// ÇëÇóÉý¼¶
...
/*
	ÊÕµ½BeaconÊý¾Ý,ÊÕµ½BeaconÊý¾ÝÖ®ºóµÄ500ms¿ªÊ¼¸üÐÂÊý¾Ý
	UUID
	Major
	Minor
	RSSI
...
static Bool _ConvertBleAdvToBeacon(BLEAdvContext *pAdvCntext, BeaconCell *pBecaon )
{
	...
	for( i = 0 ; i < 62 ; )
	{
		...
		switch ( t )
		{
			case 0xFF :	// ²é¿´ÊÇ·ñÎªÆ»¹ûµÄIbeacon
...
void JT808_timerProc()
{
	...
	else
	{
		if( GET_TICKS() - gCanbusRevTimeMS > _CAN_BUS_REV_TIMEOUT_MS )
		{
			...
			g_Jt.devState.cnt = 0x00 ;
			g_pdoInfo.isCANOk = 0;
			if( ++rev_timeout_cnt > 12 )		// 1·ÖÖÓÎ´ÊÕµ½CANÊý¾ÝÔòÈÏÎªÃ»ÓÐÁ¬½Ó
			{
				if( !gJT808Disconnect )
				{
	...
	if( ComModeSleep() )	// Ä£¿éÒÑ¾­ÐÝÃß---¼ì²âÊÇ·ñÐèÒª»½ÐÑ´¦Àí
	{
		...
		else
		{
			...
			if( !WorkMode_Sleep() && _JT808_EXT_SLEEP != gJT808ExtStatus )	// ÍâÎ§»½ÐÑ
			{
				...
				JT808_Work();
				if( gJT808ExtStatus == _JT808_EXT_WAKUP_IRQ )	// ÍâÖÃÄ£¿éÖ÷¶¯»½ÐÑ
				{
					Pms_postMsg(PmsMsg_GPRSIrq, 0, 0);
				}
			}
...
/*
	JT808ÐÝÃßÊ±---Ö÷MCU¿ÉÄÜÐÝÃß( WorkMode_Sleep() ),Ò²¿ÉÄÜ²»ÐÝÃß( !WorkMode_Sleep())
	JT808¿ÉÄÜ±»ÒÔÏÂÇé¿öÊÇ»½ÐÑ£ºÍâÎ§Ä£¿éÖ÷¶¯»½ÐÑ,±»MCU»½ÐÑ(MCUÎ´ÐÝÃßÊ±»½ÐÑ,MCUÐÝÃßÊ±»½ÐÑ)

	// »¹ÓÐÒ»ÖÖ¿ÉÄÜ,Ä£¿éÃ»ÓÐ½Ó or Ä£¿é»µÁË
*/
void JT808_run(void)
{
	...
	if( lastJT808Disconnect )		// CAN Î´Á¬½Ó
	{
		// do nothing
		Fsm_SetActiveFlag(AF_JT808, False );
	}
